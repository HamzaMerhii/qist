...
def sales_report(request):
    ...
    totals = sales.aggregate(
        ...
        tax=Sum("tax"), total=Sum("total_amount"),
    )

    by_status = sales.values("status").annotate(count=Count("id"), total=Sum("total_amount")).order_by("status")
...
def installment_report(request):
    ...
    totals = installments.aggregate(count=Count("id"), amount=Sum("amount_due"))
    by_status = installments.values("status").annotate(count=Count("id"), amount=Sum("amount_due")).order_by("status")
    ...
    return render(request, "installment_report.html", {
        ...
        "status_choices": InstallmentPayment.STATUS_CHOICES,
    })
...
def outstanding_report(request):
    ...
    for plan in plans:
        ...
        if remaining > 0:
            total_paid = plan.total_amount - remaining
            rows.append({
                "customer": plan.sale.customer,
                "plan": plan,
                "total_financed": plan.total_amount,
...
def collection_report(request):
    ...
    ).select_related("plan__sale__customer").order_by("-paid_date")

    totals = payments.aggregate(count=Count("id"), total=Sum("amount_due"))

    return render(request, "collection_report.html", {
        "start": start, "end": end, "payments": payments, "totals": totals,
    })
